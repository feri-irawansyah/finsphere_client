<script>
    import ModalCreate from "$lib/components/molecules/modals/ModalAppsApiLogs.svelte";
    ...
    <div class="row" id={data.tableName} data-aos="fade-left">
        <div class="col-12">
            <ClientGrid
                ...
                on:doubleClicked={(e) =>
                    modalStore.open(`modal-${data.tableName}`, "Detail Apps Api Logs", "", {
                        actions: "update",
                        uid: e.detail.appApiLogUid,
                        isFormViewOnly: true,
                    })}
            />
        </div>
